...
c_ruleset = {
    ...
    #       is that analyzer doesn't have type information, and ">>" is safe with
    #       many other types.
...
def display_header():
    ...
    if not displayed_header:
        if output_format:
            ...
            print('')
            print('<meta name="keywords" lang="en" content="analyzer results, security scan">')
            print("</head>")
            print("<body>")
            print("<h1>Results</h1>")
...
def process_file_args(files, patch_infos):
    ...
    # (i.e. through recursing into a directory) because analyzer
    ...
    for f in files:
        if (not allowlink) and os.path.islink(f):
            ...
            # At one time analyzer used os.path.walk, but that Python
...
def usage():
    print("""
sourcecode_analyzer [--help | -h] [--version] [--listrules]
  ...
  The options cover various aspects of source code analyzer as follows.
  ...
  --error-level=LEVEL
              ...
              especially if you mark known-okay lines as "analyzer: ignore".
              Usually you want level to be fairly high, such as 4 or 5.
              By default, it returns 0 (true) on a successful run.
